...
//#define USERID 1823605493 //Dalan
...
int main(int argc, char *argv[]) {
  ...
  unsigned char initialBuf[8] = { 0, (uintptr_t)atoi(argv[3]), 0x6c, 0xb3, 0x59, 0x29, (uintptr_t)(atoi(argv[4]) >> (8*1)) & 0xff, (uintptr_t)(atoi(argv[4]) >> (8*0)) & 0xff };
  //unsigned char initialBuf[8] = { 0, (uintptr_t)atoi(argv[3]), 0x6c, 0xb3, 0x59, 0x29, (uintptr_t)(atoi(argv[4]) >> (8*1)) & 0xff, (uintptr_t)(atoi(argv[4]) >> (8*0)) & 0xff };
  ...
  while (1) {
    ...
    // OPCODE 2
      ...
      inet_ntop(AF_INET, &ipv4addr_local_initial.sin_addr, ip, sizeof(ip));
      globalLocalPort = ntohs(ipv4addr_local_initial.sin_port);
      printf("!!!(PORT)!!! Updated Port: %d(%x)", globalLocalPort, globalLocalPort);
    }
